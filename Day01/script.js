...
const input = require('fs').readFileSync("input.txt", 'utf8').replace(/\r/g, '');

let l1 = input.split('\n').map(e=>e.split("   ")[0]).map(Number).sortf()
let l2 = input.split('\n').map(e=>e.split("   ")[1]).map(Number).sortf()

console.log("Total Difference: " + l1.map((e,i)=>Math.abs(e-l2[i])).sum())

let occ = l2.reduce((acc,curr)=>{acc[curr]=acc[curr]+1||1;return acc},{})

console.log("Total Similarity Score: " + l1.map(e=>e*occ[String(e)]||0).sum())
